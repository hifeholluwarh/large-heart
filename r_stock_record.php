...
<table width="100%" height="181" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  <tr>
   <td height="13" valign="middle" class="textonly3"><p align="center" class="title style6"><strong> Store Tally Record [<a href="print_setup_item.php" target="_blank">Stock Record</a>]           </strong></p>
          <hr /></td>
  </tr>
  <tr>
  ...
  <option value="asc" <?php echo $a;?> >asc</option>
	  ...
	  <?php 
		...
		elseif($m=="Dec") $m1="12";
			...
			if ($rept=="dd" or !$rept) echo $hd="Summary of $dcat used on ".date("l - jS \of M Y", mktime(0,0,0,$m1,$d,$y)); 
			elseif ($rept=="mm") echo $hd="Summary of $dcat used in the Month of $m, $y"; 
			elseif ($rept=="yy") echo $hd="Summary of $dcat used in the Year $y"; 
			elseif ($rept=="aa") echo $hd="Overall Summary of $dcat used till date"; 
			elseif ($rept=="ww")
			{
				echo $hd="Summary of $dcat used from [".date("D - jS M Y", mktime(0,0,0,$m1,$d-6,$y))."] 
      ...
      <table width="95%" height="26" border="1" align="center" cellpadding="5" cellspacing="0">
        <tr bgcolor="green" class="textonly3">
		  ...
		  <td width="83" align="center" bgcolor="#000000"><div align="center" class="style9"><strong>Area In Stock</strong></div></td>
		  <td width="77" align="center" bgcolor="#000000"><div align="center" class="style9"><strong>Size</strong></div></td>
...
<?php
	...
	if(!$rept or $rept=='dd') $sql="select * from $tb_name where dat='$datd' $add_this order by id $st";
	elseif($rept=='mm') $sql= "select * from $tb_name where substring(dat,1,7)='$datm' $add_this order by id $st";
	elseif($rept=='yy') $sql= "select * from $tb_name where substring(dat,1,4)='$daty' $add_this order by id $st";
	elseif($rept=='aa') $sql="select * from $tb_name $add_this order by id $st";
	elseif($rept=='ww') $sql="select * from $tb_name where dat BETWEEN '$start' AND '$end' $add_this order by id $st";
	...
	if($num_results>=1)
	{
		for($i=1; $i<=$num_results; $i++) 
		{
			...
			if ($in_stock<=$now_stock) echo "<td>".number_format($area,2)."</td>"; 
			else echo "<td>(".number_format($area,2).")</td>";
			echo "<td align='right'>".number_format($rows["price"],2)."</td>";
			$amount=$qty*$price;
			echo "<td align='right'>".number_format($area*$price,2)."</td>";	
		}
	}
...
?>
      ...
      <table width="147" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td width="43" height="25" bgcolor="#FFFFFF"><a href="print_stock_record.php?rept=<?php echo $rept;?>&amp;d=<?php echo $d;?>&amp;m1=<?php echo $m1;?>&amp;y=<?php echo $y;?>&amp;hd=<?php echo $hd;?>&amp;start=<?php echo $start;?>&amp;end=<?php echo $end;?>&amp;cat=<?php echo $cat;?>&amp;item_id=<?php echo $item_id;?>&amp;st=<?php echo $st;?>" target="_blank"><img src="images/Preview-icon.png" alt="Preview in Printable Format" width="42" height="42" border="0" /></a><a href="print_setup_item.php?download=excel" target="_blank"></a></td>
          <td width="63" bgcolor="#FFFFFF">&nbsp;</td>
          <td width="41" bgcolor="#FFFFFF"><a href="print_stock_record.php?rept=<?php echo $rept;?>&d=<?php echo $d;?>&m1=<?php echo $m1;?>&y=<?php echo $y;?>&hd=<?php $hd;?>&start=<?php echo $start;?>&end=<?php echo $end;?>&cat=<?php echo $cat;?>&item_id=<?php echo $item_id;?>&st=<?php echo $st;?>&download=<?php echo "$page".date('Ymdhms');?>"><img src="images/Download-Excel-icon.png" alt="Download in Excel Format" width="41" height="39" border="0" /></a></td>
        </tr>
      </table></td>
  </tr>
</table>
